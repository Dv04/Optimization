...
for hour in range(NUM_HOUR):
    for a in range(NUM_SOLUTION):
        ...
        while (
            sixth_value > capacity_dict["max_capacity"][NUM_DER - 1] or sixth_value < capacity_dict["min_capacity"][NUM_DER - 1]
        ):
            t = 0

            for j in range(NUM_DER - 1):
                population[a, j] = (
                    np.random.rand() * (capacity_dict["max_capacity"][j] - capacity_dict["min_capacity"][j])
    ...
    for itr in range(NUM_ITERATIONS):
        ...
        for i in range(NUM_SOLUTION):
            ...
            elif population[i, NUM_DER - 1] > capacity_dict["max_capacity"][NUM_DER - 1]:
