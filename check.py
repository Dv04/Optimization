...
def check_output():
    with open("best_solution_new.csv", newline="") as csvfile:
        ...
        for i in range(len(data) - 1):
            if (capacity_dict["min_capacity"][i % 6] <=  data[i] <= capacity_dict["max_capacity"][i % 6]) and data[i] > 0:
                print("\033[92m" + "True for index: ", i, " | ", data[i], " | Wanted: [", capacity_dict["min_capacity"][i % 6], ",", capacity_dict["max_capacity"][i % 6], "]" + "\033[0m")
            else:
                print("False for index: ", i, " | ", data[i], " | Wanted: [", capacity_dict["min_capacity"][i % 6], ",", capacity_dict["max_capacity"][i % 6], "]")
                falseCtn += 1
